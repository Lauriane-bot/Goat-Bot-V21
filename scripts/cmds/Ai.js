...
module.exports = {
  ...
  start: async function({ text, api, event, reply, react }) {
    try {
      ...
      if (!result) {
        react("❌");
        return api.editMessage('The AI response was undefined. Please try again.', heru.messageID);
      }

      const responseTime = (endTime - startTime) / 1000; // in seconds
      react("✅");

      return api.editMessage(`🇨🇮 | ${model.toUpperCase()} 𝐌𝐨𝐝𝐞𝐥 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞\n━━━━━━━━━━━━━━━━━━\n${result}\n━━━━━━━━━━━━━━━━━━\n⏰ 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞 𝐓𝐢𝐦𝐞: ${responseTime.toFixed(2)} seconds`, heru.messageID);
    } catch (error) {
      react("❌");
      return api.editMessage(`An error occurred: ${error.message}`, heru.messageID);
    }
  }
};
